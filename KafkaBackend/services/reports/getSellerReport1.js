...
let getSellerReport1 = async (msg, callback) => {
  try {
    ...
    connection_sql.query(
      `select seller_email_id, Product_id, sum(TotalPrice) as totalProductPrice, sum(quantity) as totalProductQuantity from OrderProducts where seller_email_id=? and cancelProduct=0 group by seller_email_id,Product_id`,
